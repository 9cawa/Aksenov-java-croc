...
public interface BlackListFilter {
    ...
    static void filterComments(List<String> comments, Set<String> blackList) {
        List<String> banComments = new ArrayList<>();
        for (String comment : comments) {
            for (String banWrd : blackList) {
                if (comment.toUpperCase().contains(banWrd))
                    banComments.add(comment);
            }
        }

        for (String banComment : banComments) {
            comments.remove(banComment);
        }
    }
